...
const FeedbackGraphState = GraphAnnotation.Root({
  ...
  taskType: GraphAnnotation<string>(),
  question: GraphAnnotation<string>(),
  locale: GraphAnnotation<string>(),
  ...
  taskResponseFeedback: GraphAnnotation<DimensionFeedback | null>({
    reducer: (_left, right) => right,
    default: () => null,
  }),
  coherenceFeedback: GraphAnnotation<DimensionFeedback | null>({
    reducer: (_left, right) => right,
    default: () => null,
  }),
  lexicalFeedback: GraphAnnotation<DimensionFeedback | null>({
    reducer: (_left, right) => right,
    default: () => null,
  }),
  grammarFeedback: GraphAnnotation<DimensionFeedback | null>({
    reducer: (_left, right) => right,
    default: () => null,
  }),
  scores: GraphAnnotation<IeltsScores | null>({
    reducer: (_left, right) => right,
    default: () => null,
  }),
...
async function prepareContext(state: FeedbackGraphStateType): Promise<FeedbackGraphUpdate> {
...
async function generateCriterionFeedback(
...
async function generateTaskResponseFeedback(
  state: FeedbackGraphStateType,
): Promise<FeedbackGraphUpdate> {
  return { taskResponseFeedback: await generateCriterionFeedback(state, "task_response") };
}

async function generateCoherenceFeedback(
  state: FeedbackGraphStateType,
): Promise<FeedbackGraphUpdate> {
  return {
    coherenceFeedback: await generateCriterionFeedback(state, "coherence_cohesion"),
  };
}

async function generateLexicalFeedback(
  state: FeedbackGraphStateType,
): Promise<FeedbackGraphUpdate> {
  return { lexicalFeedback: await generateCriterionFeedback(state, "lexical_resource") };
}

async function generateGrammarFeedback(
  state: FeedbackGraphStateType,
): Promise<FeedbackGraphUpdate> {
  return { grammarFeedback: await generateCriterionFeedback(state, "grammar") };
}
...
async function scoreIeltsBands(state: FeedbackGraphStateType): Promise<FeedbackGraphUpdate> {
  const options = {
    taskType: state.taskType as FeedbackRequestOptions["taskType"],
    question: state.question,
    locale: (state.locale === "en" ? "en" : "zh") as FeedbackRequestOptions["locale"],
  ...
  return { scores: parsed as IeltsScores };
}

function attachScoresAndPriority(state: FeedbackGraphStateType): FeedbackGraphUpdate {
...
function routeAfterValidation(state: FeedbackGraphStateType): string {
  return state.validationError ? "repair_feedback" : "score_ielts_bands";
}
...
const feedbackGraph = new StateGraph(FeedbackGraphState)
  ...
  .addNode("task_response_feedback", generateTaskResponseFeedback)
  .addNode("coherence_cohesion_feedback", generateCoherenceFeedback)
  .addNode("lexical_resource_feedback", generateLexicalFeedback)
  .addNode("grammar_feedback", generateGrammarFeedback)
  ...
  .addNode("score_ielts_bands", scoreIeltsBands)
  .addNode("attach_scores_and_priority", attachScoresAndPriority)
  .addNode("return_feedback", returnFeedback)
  .addEdge(START, "prepare_context")
  .addEdge("prepare_context", "task_response_feedback")
  .addEdge("prepare_context", "coherence_cohesion_feedback")
  .addEdge("prepare_context", "lexical_resource_feedback")
  .addEdge("prepare_context", "grammar_feedback")
  .addEdge(
    [
  ...
  .addEdge("score_ielts_bands", "attach_scores_and_priority")
  .addEdge("attach_scores_and_priority", "return_feedback")
